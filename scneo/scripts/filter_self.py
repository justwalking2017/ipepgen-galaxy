...
def main() -> None:
    ...
    proteins = [str(record.seq).upper() for record in SeqIO.parse(args.proteome, "fasta")]
    rows = []
    for row in read_tsv(args.candidates):
        row["reference_match"] = "yes" if any(row["peptide"].upper() in protein for protein in proteins) else "no"
